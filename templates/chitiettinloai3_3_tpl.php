<?php 
			...
			$id_item = $result_detail['id_item'];
			$id_cat  = $result_detail['id_cat'];
			$sql_tinll="select * from #_product_list where hienthi =1 order by stt asc";
			$d->query($sql_tinll);	
			$result_tinll=$d->result_array();	
			
			
			$d->reset();
			$sql_tinlq="select * from #_product where  id<>'$id' and ( id_list = '$id_list' or id_item = '$id_item' or id_cat = '$id_cat') limit 0,3 ";
			$d->query($sql_tinlq);	
			$result_tinlq=$d->result_array();
			
			$d->reset();
			$sql_tinnb="select * from #_product where  noibat = '1' limit 0,3 ";
			$d->query($sql_tinnb);
			$result_tinnb=$d->result_array();
?>
	...
	<div class="news_wrap wr-page dt">
		<div class="box_news_detail">
			<div class="news_content">
				...
				<div class="content">
					<p style="text-align:justify"><?=$result_detail['mota_en']?></p>
			...
			<div class="news_sidebar">
				<div class="box_list">
					...
					<div class="list_item new-structure">
                               <?php 
					   for ($i = 0; $i < count($result_tinnb); $i++) 
					   {
					?>
					<div class="item">
				        <a href="tin-tuc-detail/<?=$result_tinnb[$i]["tenkhongdau"]?>-<?=$result_tinnb[$i]["id"]?>.html" title=""><img src="upload/sanpham/<?=$result_tinnb[$i]["thumb"]?>" alt="<?=$result_tinnb[$i]["ten_vi"]?>" title="<?=$result_tinnb[$i]["ten_vi"]?>"></a>
				        <div class="text">
					        <div><a href="tin-tuc-detail/<?=$result_tinnb[$i]["tenkhongdau"]?>-<?=$result_tinnb[$i]["id"]?>.html" title=""><?=$result_tinnb[$i]["ten_vi"]?></a></div>
				        </div>
				        <div class="clear"></div>
			        </div>
					<?php
				...
				<div class="box_list">
					...
					?>
					<div class="item">
				        <a href="tin-tuc-detail/<?=$result_tinlq[$i]["tenkhongdau"]?>-<?=$result_tinlq[$i]["id"]?>.html" title=""><img src="upload/sanpham/<?=$result_tinlq[$i]["thumb"]?>" alt="<?=$result_tinlq[$i]["ten_vi"]?>" title="<?=$result_tinlq[$i]["ten_vi"]?>"></a>
				        <div class="text">
