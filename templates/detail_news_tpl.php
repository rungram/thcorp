<?php 
			...
			$sql_detail="select * from #_slideshow where id='$id'";
			$d->query($sql_detail);	
			$result_detail=$d->fetch_array();
			$loai = $result_detail['loai'];
			$d->reset();
			$sql_tinlq="select * from #_slideshow where  id<>'$id' and loai = '$loai' limit 0,3 ";
			$d->query($sql_tinlq);	
	...
	<div class="news_wrap wr-page dt">
		<div class="box_news_detail">
			<div class="news_content">
				...
				<h2 class="title b"><?=$result_detail['ten']?></h2>
				<div class="tagline">
					<div class="share right">
				...
				<div class="content">
					<p style="text-align:justify"><?=$result_detail['noidung']?></p>
			...
			<div class="news_sidebar">
				<div class="box_list">
					...
					?>
					<div class="item">
				        <a href="noi-dung-gioi-thieu/<?=$result_tinlq[$i]["tenkhongdau"]?>-<?=$result_tinlq[$i]["id"]?>.html" title=""><img src="<?= _upload_slideshow_l.$result_tinlq[$i]['photo'] ?>" alt="<?= $result_tinlq[$i]['ten'] ?>" title="<?= $result_tinlq[$i]['ten'] ?>"></a>
				        <div class="text">
					        <div><a href="noi-dung-gioi-thieu/<?=$result_tinlq[$i]["tenkhongdau"]?>-<?=$result_tinlq[$i]["id"]?>.html" title=""><?=$result_tinlq[$i]["ten"]?></a></div>
				        </div>
				        <div class="clear"></div>
			        </div>
					<?php
