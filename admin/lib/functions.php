...
#
function fns_Rand_digit2($min,$max,$num)
	{
		$result='';
		for($i=0;$i<$num;$i++){
			$result.=rand($min,$max);
...
function upload_mutil_file($arr_filename,$path_upload_file) {
	$files = array();
	
	if(count($_FILES[$arr_filename]['name']) > 0){
					//Loop through each file
					
					for($i=0; $i<count($_FILES[$arr_filename]['name']); $i++) {
					  //Get the temp file path
					    $file_name   = fns_Rand_digit2(0,9,15);
						$tmpFilePath = $_FILES[$arr_filename]['tmp_name'][$i];
						//Make sure we have a filepath
						if($tmpFilePath != ""){
							//save the filename
							$shortname = $_FILES[$arr_filename]['name'][$i];
							$ext = end(explode('.',$shortname));
							$img_name = $file_name .'.'. $ext;
							//save the url and the file
							$filePath = $path_upload_file . $img_name;
	...
	function paging($r, $url='', $curPg=1, $mxR=5, $mxP=5, $class_paging='')
	{
		...
		if($totalRows>$mxR)
		{
			...
			#{
				...
				$paging .=" <a href='".$url."&curPage=".($curPg-1)."' class=\"{$class_paging}\" >Previous</a> "; //ve truoc
			#}
			$paging.=$paging1; 
			#if(((int)(($curPg-1)/$mxP+1)*$mxP) < $totalPages)  
			#{
				#$paging .=" <a href='".$url."&curPage=".($end+1)."' class=\"{$class_paging}\" >&#8250;</a> "; //ke
				$paging .=" <a href='".$url."&curPage=".($curPg+1)."' class=\"{$class_paging}\" >Next</a> "; //ke
		...
		if($totalRows>$mxR)
		{
			...
			{	
				if(($i>((int)(($curPg-1)/$mxP))* $mxP) && ($i<=((int)(($curPg-1)/$mxP+1))* $mxP))
				{
					if($start==1) $start=$i;
					if($i==$curPg){
						$paging1.=" <span class='page current'>".$i."</span> ";//dang xem
					} 		  	
					else    
					{
